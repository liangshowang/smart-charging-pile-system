...
/* 引脚编号 (物理封装引脚号 → drvp_gpio 接口) */
#define PIN_ELC0  26   /* M2  */
#define PIN_ELC1  3    /* B12 */
...
static int cmd_led(int argc, char **argv)
{
    ...
    if (strcmp(argv[1], "on") == 0) {
        drvp_gpio->write(PIN_SLED, PIN_HIGH);
        printf("LED ON\r\n");
    } else if (strcmp(argv[1], "off") == 0) {
        drvp_gpio->write(PIN_SLED, PIN_LOW);
        printf("LED OFF\r\n");
    } else {
...
static int cmd_relay(int argc, char **argv)
{
    ...
    /* 解析继电器编号 */
    {
        ...
        if (strcmp(argv[2], "on") == 0) {
            drvp_gpio->write(rly_pin, PIN_HIGH);
            printf("%s ON\r\n", rly_name);
        } else if (strcmp(argv[2], "off") == 0) {
            drvp_gpio->write(rly_pin, PIN_LOW);
            printf("%s OFF\r\n", rly_name);
        } else {
            printf("Invalid: %s (expect on|off)\r\n", argv[2]);
            return -1;
