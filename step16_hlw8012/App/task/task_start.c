...
static void task_entry(void *arg)
{
    ...
    /* ---- 9. 创建 host 任务 (LED 刷新 + 命令行 + 灯板动画) ---- */
    task_host->create(NULL, NULL);

    /* ---- 10. 创建 listen 任务 (HLW8012 脉冲轮询) ---- */
    task_listen->create(NULL, NULL);

    /* ---- 11. 创建 ctrl 任务 (占位 — 后续充电控制 + OTA) ---- */
    task_ctrl->create(NULL, NULL);

    /* ---- 12. 主循环 (占位 — 后续 Step 21 网络调度) ---- */
    while (1) {
        vTaskDelay(5);
    }
}
