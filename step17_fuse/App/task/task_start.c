...
static void show_banner(void)
{
    printf("\r\n");
    printf("========================================\r\n");
    printf("  Step 17: Fuse Detection (optocoupler 50Hz)\r\n");
    printf("  CPU Clock : %d Hz\r\n", SystemCoreClock);
    printf("  HC595 Bus : DAT=P1(36) CLK=P0(35) UD=P2(37)\r\n");
    printf("  HLW8012   : HLW0=C3(22) HLW1=C7(20) (falling edge ISR)\r\n");
    printf("  Fuse      : PIN=C2(21) (optocoupler pulse, <10/sec = broken)\r\n");
    printf("========================================\r\n");
    printf("\r\n");
}
...
static void task_entry(void *arg)
{
    ...
    /* ---- 10. 创建 host 任务 (LED 刷新 + 命令行 + 灯板动画) ---- */
    task_host->create(NULL, NULL);

    /* ---- 11. 创建 listen 任务 (保险丝检测 + HLW8012 脉冲轮询) ---- */
    task_listen->create(NULL, NULL);

    /* ---- 12. 创建 ctrl 任务 (占位 — 后续充电控制 + OTA) ---- */
    task_ctrl->create(NULL, NULL);

    /* ---- 13. 主循环 (占位 — 后续 Step 21 网络调度) ---- */
    while (1) {
        vTaskDelay(5);
    }
}
