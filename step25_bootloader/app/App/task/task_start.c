...
static void task_entry(void *arg)
{
    ...
    /* ---- Step 25: 写入启动标志到 CONF 扇区 (0x10000) ----
     * BootLoader 上电后读取此标志决定是否跳转 APP。
     * 标志值由 flash_partition.h 定义, 统一管理分区地址。 */
    {
        Flash_Param_at_xMHz(120);

        uint32_t *p = (uint32_t *)PART_CONF_START;

        if (p[0] != PART_CONF_MAGIC) {
            uint32_t buf[4];
            buf[0] = PART_CONF_MAGIC;  /* 启动标志 */
            buf[1] = 0xFFFFFFFF;       /* 保留 */
            buf[2] = 0xFFFFFFFF;       /* 保留 */
            buf[3] = 0xFFFFFFFF;       /* 保留 (对齐填充) */

            FLASH_Erase(PART_CONF_START);
            FLASH_Write(PART_CONF_START, buf, 4);
            sysprt->alog("[task_start] Boot flag written "
                         "(0x%08X @0x%08X)\r\n",
                         PART_CONF_MAGIC, PART_CONF_START);
        } else {
            sysprt->alog("[task_start] Boot flag already valid "
                         "(0x%08X)\r\n", PART_CONF_MAGIC);
        }
    }
