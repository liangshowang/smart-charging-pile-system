...
typedef struct {
    ...
    /* AT+CGMI — 查询制造商信息, result 返回厂商字符串 */
    int (*CGMI)(void *dev, char *result, int maxlen);

    /* AT+CGMR — 查询固件版本, result 返回版本字符串 */
    int (*CGMR)(void *dev, char *result, int maxlen);

    /* AT+CIPMODE=<mode> — 设置透传模式, mode=0非透传/1透传 */
    int (*CIPMODE)(void *dev, int mode);

    /* AT+CIPMUX=<mode> — 设置连接模式, mode=0单连接/1多连接 */
    int (*CIPMUX)(void *dev, int mode);

    /* AT+CSTT — 设置 APN, apn/user/pwd 为 NULL 则自动获取 */
    int (*CSTT)(void *dev, const char *apn, const char *user, const char *pwd);

    /* AT+CIICR — 激活 GPRS 移动场景 (获取 IP), 耗时较长(可达30s) */
    int (*CIICR)(void *dev);

    /* AT+CIFSR — 查询本机 IP 地址, ip 返回点分十进制字符串 */
    int (*CIFSR)(void *dev, char *ip, int maxlen);

    /* AT+CIPSTART — 建立 TCP/UDP 连接
     *   type: "TCP" 或 "UDP", addr: 目标 IP/域名, port: 端口 */
    int (*CIPSTART)(void *dev, const char *type, const char *addr, const char *port);

    /* AT+CIPCLOSE — 关闭连接, id=连接号(通常0) */
    int (*CIPCLOSE)(void *dev, int id);
