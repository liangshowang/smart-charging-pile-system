...
static void show_banner(void)
{
    printf("\r\n");
    printf("========================================\r\n");
    printf("  Step 14: Thread-Safe Logging (sysprt)\r\n");
    printf("  CPU Clock : %d Hz\r\n", SystemCoreClock);
    printf("  Log Queue : %d msgs × %d bytes\r\n", SYSPRT_QUEUE_LEN, SYSPRT_BUF_SIZE);
...
static void task_entry(void *arg)
{
    ...
    /* ---- 6. 创建 host 任务 (LED 刷新 + 命令行) ---- */
    task_host->create(NULL, NULL);

    /* ---- 7. 创建 listen 任务 (占位 — 后续 HLW8012 计量) ---- */
    task_listen->create(NULL, NULL);

    /* ---- 8. 创建 ctrl 任务 (占位 — 后续充电控制 + OTA) ---- */
    task_ctrl->create(NULL, NULL);

    /* ---- 9. 主循环 (占位 — 后续 Step 21 网络调度) ---- */
    while (1) {
        vTaskDelay(5);
    }
}
