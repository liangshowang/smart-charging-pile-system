...
static void show_banner(void)
{
    printf("\r\n");
    printf("========================================\r\n");
    printf("  Step 18: FlashDB Persistent Storage\r\n");
    printf("  CPU Clock : %d Hz\r\n", SystemCoreClock);
    printf("  FlashDB   : sector 0x%06X (4KB), IAP write\r\n", FLASHDB_ADDR);
    printf("  Storage   : boot_code/app/ota/net/k[2]/b[2]/user/pwd\r\n");
    printf("========================================\r\n");
    printf("\r\n");
}
...
static void task_entry(void *arg)
{
    ...
    /* ---- 11. 创建 host 任务 (LED 刷新 + 命令行 + 灯板动画) ---- */
    task_host->create(NULL, NULL);

    /* ---- 12. 创建 listen 任务 (保险丝检测 + HLW8012 脉冲轮询) ---- */
    task_listen->create(NULL, NULL);

    /* ---- 13. 创建 ctrl 任务 (占位 — 后续充电控制 + OTA) ---- */
    task_ctrl->create(NULL, NULL);

    /* ---- 14. 主循环 (占位 — 后续 Step 21 网络调度) ---- */
    while (1) {
        vTaskDelay(5);
    }
}
