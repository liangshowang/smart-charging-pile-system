...
static void show_banner(void)
{
    printf("\r\n");
    printf("========================================\r\n");
    printf("  Step 21: Network State Machine (net_man)\r\n");
    printf("  CPU Clock : %d Hz\r\n", SystemCoreClock);
    printf("  UART0     : 115200 bps (debug console)\r\n");
    printf("  UART1     : 115200 bps (AIR724 4G module)\r\n");
    printf("  Stack     : net_man → air_man → at_dev → uart1\r\n");
...
static void task_entry(void *arg)
{
    ...
    while (1) {
        ...
        if (net_man->is_ready()) {
             ...
             *
             * Step 21 到此达成目标:
             *   - TCP 连接 www.armsoc.cn:9002 成功
             *   - 收到服务器 Logon 响应
             *
             * 后续 Step 22 将在此基础上建立服务器协议层
             * (心跳 / 命令派发 / 事件上报)。
             */
            ...
            sysprt->alog("[task_start] Phase=%d Step=%d Reconn=%d\r\n",
                         phase, step, reconnect);
            sysprt->alog("[task_start] ===========================\r\n");
