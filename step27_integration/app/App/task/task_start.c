...
static void task_entry(void *arg)
{
     ...
     * 首次上电时 CONF 未初始化, 写入默认配置 (boot_app=_APP_FAC)。 */
    {
        Flash_Param_at_xMHz(120);

        volatile boot_config_t *p = BOOT_CFG_PTR;

        if (p->boot_code != BOOT_CODE_MAGIC) {
            boot_config_t cfg;
            cfg.boot_code = BOOT_CODE_MAGIC;
            cfg.boot_app  = _APP_FAC;
            cfg.ota_req   = 0;
            ...
            FLASH_Write(PART_CONF_START, (uint32_t *)&cfg,
                        (sizeof(cfg) + 3) / 4);
            sysprt->alog("[task_start] boot_config_t written "
                         "(boot_app=%lu @0x%08X)\r\n",
                         cfg.boot_app, PART_CONF_START);
        } else {
            sysprt->alog("[task_start] boot_config_t already valid "
                         "(boot_app=%lu)\r\n", p->boot_app);
        }
    }
